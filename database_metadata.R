...
pp <- RSQLite::dbGetQuery(
    conn = conn,
    "SELECT *  FROM PP  WHERE STUDYID='2688-006'"
)
...
for (i in 1:nrow(ind_table)) {
	...
	df <- get_pp(conn, studyid)
	num_row [i]  <- nrow(df)
	if (nrow(df) != 0) {
	USUBJID[i] <- ifelse(nrow(df) == sum(df$USUBJID == ""), "empty", ifelse(
		nrow(df) == sum(df$USUBJID != ""), "populated", ifelse(all(is.na(df$USUBJID)), NA, "mixed")
	))

	POOLID[i] <- ifelse(nrow(df) == sum(df$POOLID == ""), "empty", ifelse(
		nrow(df) == sum(df$POOLID != ""), "populated", ifelse(all(is.na(df$POOLID)), NA, "mixed")
	))
...
#######################################################
ind_table[, num_row_co_2 := num_row]
ind_table
num_row
USUBJID
POOLID
