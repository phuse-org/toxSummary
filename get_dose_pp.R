...
get_pk_param <- function(conn, studyid, pk_param="AUCLST", sex_include="ALL"){
  ...
  #dose_wide$TRTDOS <- as.numeric(dose_wide$TRTDOS)
  # select only required columns
  pp_domain <- data.table::as.data.table(pp_domain)
  pp_domain <-  pp_domain[, .(STUDYID, DOMAIN,USUBJID,POOLID,PPTESTCD,PPTEST,
                              PPORRES,PPORRESU,PPSTRESC,PPSTRESU,PPSTRESN)]
  ...
  if (nrow(pp_domain) == sum(pp_domain$USUBJID!="")) {
    ...
    pooldef_list <- RSQLite::dbGetQuery(conn = conn, 'SELECT DISTINCT STUDYID FROM POOLDEF')
    ...
    pooldef <- RSQLite::dbGetQuery(conn = conn, 'SELECT * FROM POOLDEF WHERE STUDYID==:x', params = list(x=studyid))
    ...
    pooldef <- RSQLite::dbGetQuery(conn = conn, 'SELECT * FROM POOLDEF WHERE STUDYID==:x',
                                   params = list(x=studyid))
    pooldef <- data.table::as.data.table(pooldef)
