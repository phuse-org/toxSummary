...
## ----------   --------------------  ------------------------------------------
## 2020-06-29   Yousuf Ali,           Initial version
##              Kevin Snyder
##              
################################################################################
...
#' 		where app will be running. Default is local
#' 
...
#' @examples 
#' \dontrun{
#' toxSummary_app(database_path = "path/of/your/database.db",studyid_file = "path/for/IND_studyid_mapping.csv", save_file_path = NULL)
#' }
#' 
...
data_modal <- function() {
  ...
  shiny::observeEvent(input$edit_nonclinical, {
	  shiny::req(input$selectData)
      input$selectData
      Data <- getData()
      studyList <- names(Data[["Nonclinical Information"]])
